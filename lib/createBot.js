...
module.exports = (options = {}) => {
    ...
    bot._client.once('connect_allowed', () => {
        ...
        bot.bedrock ={}
        require('./plugins/bedrock/activate_block')(bot, options)

        require('./plugins/world/world')(bot, options)
        require('./plugins/world/blocks')(bot, options)
        require('./plugins/world/signs')(bot, options)

        require('./plugins/entity/entities')(bot, options)
        require('./plugins/entity/players')(bot, options)

        const ItemStackRequest = require('./plugins/inventory/types/item_stack_request')(bot)

        require('./plugins/inventory/inventory')(bot, { ...options, ...{ ItemStackRequest } })
        require('./plugins/inventory/simple_inventory')(bot, options)
        require('./plugins/inventory/chest')(bot, options)
        require('./plugins/inventory/equip')(bot, options)
        require('./plugins/inventory/consume')(bot, options)
        ...
        require('./plugins/creative/creative')(bot, { ...options, ...{ ItemStackRequest } })
    })
    ...
    bot._client.on('load_runtime_data', () => {
        ...
        if (bot._client.startGameData.block_network_ids_are_hashes) {
            ...
            console.log('[load_runtime_data] Calculating Block State Hashes')
            bot.registry.calculateBlockStateHashes()
        }        

...
module.exports = (bot) => {
    ...
    async function updateSign(block, text, back = false) {
        ...
        //Wait a bit for entity data (Fix this)
        while (!block.entity?.value?.FrontText?.value) {

            await new Promise((resolve) => setTimeout(resolve, 200)) //Fix: needs to wait for auto close to fire

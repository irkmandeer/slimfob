...
module.exports = (bot) => {
    ...
    //TODO: Need to "debounce" block updates
    //TODO: Remove block entity if block is destroyed or moved
    //This is also a problem with the block_entity_data packet. An update_block is received,
    //blockUpdate get's emitted (which don't contain the block entity), '
    //then a block_entity_data packet is received
    //Last time I implemented this, I added a simple timer to queue block updates.
    //The timer which is reset for each block_update, if there are update queued they are 
    //processed immediately.
    //It seems you won't receive multiple block_updates without receiving block_entity_data first.
    //block_entity_data is only sent when it's a block entity. Maybe keep a list
    //of known block entities and process other blocks immediately. Or maybe build a dynamic list?
    //Since block_entity_data is received immediately, updates are quite fast, but slow if the last
    //block updates is not a block entity. 
    //Need to determine if update_subchunk_block is ever used for block entities.
    //Also, need to determine how packets look when a block is moved, eg a piston. 
    //What if it gets moved over a chunk border? Will the entity data be removed correctly with the current logic.
